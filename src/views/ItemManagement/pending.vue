<template>
  <div>
    ...
    <el-table :data="list" border style="width: 100%" height="410" v-loading="loading" element-loading-text="拼命加载中"  :header-cell-style="headClass" size="mini" >
      ...
      <el-table-column prop="" label="操作" align="center" width="95">
          <template slot-scope="scope">
            ...
            <el-tooltip class="item" effect="dark" content="上架" placement="bottom">
                <el-button circle type="success" icon="el-icon-check" size="mini" @click="shelves(scope.row.id,scope.row.itemName)"></el-button>
            </el-tooltip>
          </template>   
      </el-table-column>
    </el-table>
...
export default {
  ...
  methods: {
    ...
    soldOut(id,name) {
      ...
      .then(() => {
        this.changeItemState(id,-2)
      })
      .catch(() => {});
    },
    // 上架
    shelves(id,name) {
      this.$confirm(`是否将物品'${name}'上架？`, "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning"
      })
      .then(() => {
        this.changeItemState(id,1)
      })
      .catch(() => {});
    },

    // 上下架接口
    changeItemState(id,state) {
